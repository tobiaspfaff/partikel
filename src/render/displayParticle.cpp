...
DisplayParticle::DisplayParticle(CLQueue& queue, const Vec2& domainMin, const Vec2& domainMax, GLWindow& window) :
	window(window), domainMin(domainMin), domainMax(domainMax),
	fillKernel(queue, "display.cl", "displayParticle")
{
	// set up GL buffers
	vbPart.defineAttrib(0, GL_FLOAT, 2, 0);
	...
	// cl buffers
	vector<cl_float2> dummy(10000);
	vbPart.buffer.setData(dummy.data(), dummy.size()); // so that CL can read proper size
	clPart = make_unique<CLVertexBuffer<cl_float2> >(queue, vbPart);
